...
export default function PageProductsTenant() {
  ...
  const [loadingEditOrCreateDialog, setLoadingEditOrCreateDialog] = useState(false)
  ...
  async function handleSaveEditOrCreateDialog(event: FormEvent<HTMLFormElement>) {
    try {
      setLoadingEditOrCreateDialog(true);
      const formData = new FormData(event.currentTarget);

      if (productToEdit) {
        formData.append('id', productToEdit.id)
        await api.updateProduct(formData)
        return setEditOrCreateDialogOpen(false);
      }

      await api.createProduct(formData);
      setEditOrCreateDialogOpen(false);
      getDataApi();
    } catch (err) {
      console.log(err)
    } finally {
      setLoadingEditOrCreateDialog(false);
    }
  }
  ...
  return (
    <Box sx={{ my: 3 }}>
      ...
      <ProductEditDialog
        ...
        onSave={handleSaveEditOrCreateDialog}
        disabled={loadingEditOrCreateDialog}
